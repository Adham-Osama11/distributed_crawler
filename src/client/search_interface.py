...
def perform_search(query):
    ...
    In a production environment, this would call a dedicated search API
    provided by the indexer node. For this implementation, we'll directly
    query DynamoDB for simplicity.
    """
    try:
        ...
        if index_metadata:
            ...
            results = search_using_s3_index(query, index_metadata)
        
        # If no results from S3 index, try DynamoDB
        if not results:
            results = search_using_dynamodb(query)
...
def search_using_dynamodb(query):
    ...
    try:
        ...
        try:
            ...
            # This is a very simplified search that just looks for the query in the title
            response = table.scan(
                Limit=10
            )
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateStats() {
                fetch('/stats')
                    .then(response => response.json())
                    .then(data => {
                        document.querySelector('.stat-value:nth-child(1)').textContent = data.urls_in_frontier;
                        document.querySelector('.stat-value:nth-child(2)').textContent = data.urls_indexed;
                        document.querySelector('.stat-value:nth-child(3)').textContent = data.active_crawlers;
                    })
